    ...
    // event.inputType === 'insertReplacementText'
        ...
        if (canNativelyEdit) {
          editor.controller.tmp.nextNativeOperation = {
            ...
            slateSpanNode: window.getSelection().anchorNode.parentElement.closest(SELECTORS.KEY)
          };
        } else {
          event.preventDefault();
...
function BeforePlugin() {
  ...
  function flushQueuedNativeOperations(editor) {
    ...
    var oldSelection = nextNativeOperation.selection,
        slateSpanNode = nextNativeOperation.slateSpanNode;
    ...
    var currentSlateSpanNode = textNode.parentElement.closest(SELECTORS.KEY);
    ...
    var slateNodeFromDomNode = editor.findNode(slateSpanNode);
    if (slateNodeFromDomNode !== node) throw Error('YOWIE WOWIE: Slate nodes do not match up!');

    var allChildTextNodes = slateSpanNode.querySelectorAll(SELECTORS.STRING + ', ' + SELECTORS.ZERO_WIDTH);
    var _iteratorNormalCompletion = true;
    var _didIteratorError = false;
    var _iteratorError = undefined;
    ...
    try {
      for (var _iterator = allChildTextNodes[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        var isStringNode = childTextNode.hasAttribute(DATA_ATTRS.STRING);
        var isZeroWidth = childTextNode.hasAttribute(DATA_ATTRS.ZERO_WIDTH);
        if (isStringNode || isZeroWidth) {
          var hasZeroWidthChars = childTextNode.textContent.indexOf('\uFEFF') >= 0;
          if (isStringNode && hasZeroWidthChars || isZeroWidth && childTextNode.textContent !== '\uFEFF') {
            ...
            try {
              for (var _iterator2 = childTextNode.childNodes[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
    ...
    var newTextContent = slateSpanNode.textContent.replace(/[\uFEFF]/g, '');
