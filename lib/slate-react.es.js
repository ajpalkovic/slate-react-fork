...
function AfterPlugin() {
  ...
  function onBeforeInput(event, editor, next) {
    ...
    if (isSynthetic) {
      ...
      var canNativelyEdit = editor.value.selection.isCollapsed;
  ...
  function onInput(event, editor, next) {
    ...
    console.log(editor.value.document.text.length);
...
function BeforePlugin() {
  ...
  function onCompositionEnd(event, editor, next) {
    ...
    if (editor.controller.tmp.nextNativeOperation) {
      if (window.getSelection().anchorNode !== editor.controller.tmp.nextNativeOperation.textNode) {
        throw Error('YOWIE WOWIE: text nodes have changed!');
      }
  ...
  function flushQueuedNativeOperations(editor) {
    ...
    console.log('flushQueued');

    var oldSelection = nextNativeOperation.selection,
        textNode = nextNativeOperation.textNode;

    var _window$getSelection = window.getSelection(),
        currentTextNode = _window$getSelection.anchorNode,
    ...
    var key = oldSelection.anchor.key;
    var path = editor.value.document.getPath(key);
    var node = editor.value.document.getNode(key);
    var slateSpan = textNode.parentElement.closest('[data-key]');
    var slateNodeFromDomNode = editor.findNode(slateSpan);

    if (slateNodeFromDomNode !== node) {
      throw Error('YOWIE WOWIE: Slate nodes do not match up!');
    }

    if (textNode.textContent.indexOf('\uFEFF') >= 0) {
      textNode.textContent = textNode.textContent.replace(/[\uFEFF]/g, '');
      textNode.parentElement.removeAttribute(DATA_ATTRS.ZERO_WIDTH);
      textNode.parentElement.removeAttribute(DATA_ATTRS.LENGTH);
    }

    var allChildTextNodes = slateSpan.querySelectorAll(SELECTORS.STRING);
    console.log(allChildTextNodes);
    ...
    try {
      for (var _iterator = allChildTextNodes[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        var childTextNode = _step.value;

        console.log(childTextNode.textContent);
        console.log(childTextNode.textContent.length);

        if (childTextNode.textContent.indexOf('\uFEFF') >= 0) {
    ...
    console.log('currenOffset: ' + currentOffset);

    console.log('textNode: ' + textNode.textContent + ' ' + textNode.textContent.length);

    console.log('slateSpan: ' + slateSpan.textContent + ' ' + slateSpan.textContent.length);
    ...
    editor.insertTextAtRange(Range.create({
      ...
      focus: { path: path, key: key, offset: node.text.length }
    }), slateSpan.textContent.replace(/[\uFEFF]/g, ''));

    console.log(editor.value.document.text.length);

    console.log('flush selAfterInsert :', JSON.stringify(editor.value.selection.toJSON()));

    var point = editor.findPoint(textNode, currentOffset);
