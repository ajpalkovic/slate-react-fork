...
var Leaf = function Leaf(props) {
  ...
  if (editor.query('isVoid', parent)) {
    ...
    children = React__default.createElement(ZeroWidthString, { length: parent.text.length });
  } else if (text === '' && parent.object === 'block' && parent.text === '' && parent.nodes.last() === node) {
    // COMPAT: If this is the last text node in an empty block, render a zero-
    // width space that will convert into a line break when copying and pasting
    // to support expected plain text.
    children = React__default.createElement(ZeroWidthString, { isLineBreak: true });
  } else if (text === '') {
    // COMPAT: If the text is empty, it's because it's on the edge of an inline
    // node, so we render a zero-width space so that the selection can be
    // inserted next to it still.
    children = React__default.createElement(ZeroWidthString, null);
  } else {
...
function AfterPlugin() {
  ...
  function onBeforeInput(event, editor, next) {
    ...
    if (isSynthetic) {
      ...
      var canNativelyEdit = true;

      // Don't allow native rendering unless the selection is collapsed.
      canNativelyEdit = editor.value.selection.isCollapsed;

      if (canNativelyEdit) {
        var startTextNode = editor.value.startText;
        var selectionStartPoint = editor.value.selection.start;
      ...
      if (canNativelyEdit) {
        editor.controller.asNativeOperation(function () {
